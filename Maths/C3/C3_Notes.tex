...
\begin{enumerate}
  ...
  When r=4, $\frac{dr}{dt} = \frac{-40}{32 \pi } = \frac{-5}{4\pi}$\\
  rate of decrease of radius = $\frac{5}{4\pi} cm$\inv{s}$
\end{enumerate}
\paragraph{Exponential functions}\mbox{}\\
Exponential functions have a variable as a power.
\begin{multicols}{2}
\paragraph{Ex1}
\[Q=2\times3^t\]
\[
...
\]
\end{multicols}
\bold{Note}- If $y=a\timesb^2$ then
\begin{enumerate}
\item $y=a$ When $t=0$
\item For each equal interval of t the value of y is multiplied by the same amount.
\end{enumerate}
...
\underline{Exponential Growth/Decay} - the rate of increase/decrease of the quantity is proportional to the amount present at that time.
...
{1.2^x};
\draw (-30,90) node[anchor=north west] {$a^{x+\delta x}$};
\end{axis}
\end{tikzpicture}
This is quite hard...
